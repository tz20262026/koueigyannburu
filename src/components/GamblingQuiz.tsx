  ...
  if (finished) {
    ...
    return (
      <div className="rounded-2xl border border-[#d4af37]/30 bg-[#0f0f1a] p-6 text-center">
        ...
        <p className="text-gray-300 text-sm mb-2">正答率 {pct}%</p>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f0f1a] p-6">
      <div className="flex items-center justify-between mb-4">
        <span className="text-gray-300 text-xs">問題 {current + 1}/{questions.length}</span>
      ...
      <div className="space-y-2 mb-4">
        {q.options.map((opt, i) => {
          ...
          if (selected === null) {
            ...
            cls += "border-white/5 text-gray-300";
          }
          return (
            <button key={i} className={cls} onClick={() => handleAnswer(i)}>
              <span className="font-black mr-2 text-gray-300">{String.fromCharCode(65 + i)}.</span>
              {opt}
            </button>
          );
        })}
